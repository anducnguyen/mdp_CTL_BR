...
%  cns=[cns, z(:)>=0];
%  cns=[cns, sum(z(:))==1];
% cns=[cns, z(7)+z(23)+z(39)+z(55)>=epsilon];
% epsilon=0.45;
% % initial sample
% BR_Sample_num=50;
% 
...
%   for i=1:N
%     sample_BR{i+1}=Approx_Control(MDP,Vert_BR{i},BR_Sample_num);
...
%   %% reach-avoid
 N=50;
%% target distribution set   A={7,23,39,55}
%  cns=[];
%  cns=[cns, z(:)>=0];
%  cns=[cns, sum(z(:))==1];
% cns=[cns, z(7)+z(23)+z(39)+z(55)>=epsilon];
...
figbarreach= waitbar(0,'ReachAvoid');
  for i=1:N
    sample_BR{i+1}=Approx_Reach_Avoid(MDP,Vert_BR{i},BR_Sample_num,Obs);
    sample_BR{i+1}(find(sample_BR{i+1}<= 1.0000e-8))=0;
    for kkk=1:size(sample_BR{i+1},1)
        sample_BR{i+1}(kkk,:)=sample_BR{i+1}(kkk,:)/sum(sample_BR{i+1}(kkk,:));
    end
